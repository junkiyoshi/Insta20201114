...
void ofApp::update() {
	...
	int radius = 300;
	for (int z = -500; z <= 500; z += 6) {

		int deg_start = ofMap(ofNoise((z + 600) * 0.003 + ofGetFrameNum() * 0.01), 0, 1, -540, 540);
		int color_index = 0;
		for (int deg = deg_start; deg < deg_start + 360; deg += 120) {

			this->setRingToMesh(this->face, this->frame, glm::vec3(0, 0, z), radius, 50, 6, deg, deg + 100, color_list[color_index], color_list[color_index]);
			color_index++;
		}
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height, int deg_start, int deg_end, ofColor face_color, ofColor frame_color) {
	...
	for (int deg = deg_start; deg <= deg_end; deg += 5) {
		...
		vertices.push_back(glm::vec3((radius + width * 0.5) * cos((deg + 5) * DEG_TO_RAD), (radius + width * 0.5) * sin((deg + 5) * DEG_TO_RAD), height * -0.5));
		vertices.push_back(glm::vec3((radius + width * 0.5) * cos((deg + 5) * DEG_TO_RAD), (radius + width * 0.5) * sin((deg + 5) * DEG_TO_RAD), height * 0.5));
		vertices.push_back(glm::vec3((radius + width * 0.5) * cos(deg * DEG_TO_RAD), (radius + width * 0.5) * sin(deg * DEG_TO_RAD), height * 0.5));
